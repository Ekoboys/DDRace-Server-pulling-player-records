...
// Function to load JSON file from an URL
json get_json_from_url(const std::string &url)
{
	CURL *curl;
	CURLcode res;
...
void Datenbank(nlohmann::json eintrag, std::string name, unsigned char uuid)
{
	std::string conn = "file path of sqlite file";
	...
	int rc = sqlite3_open(conn.c_str(), &conndb);

	dbused = true;

	for(auto &[type, data] : eintrag["types"].items())
	{
		for(auto &[map_name, map_data] : data["maps"].items())
		{
			if(map_data.contains("time"))
			{
				double time = map_data["time"];

				std::string insert = std::format("insert into record_race (Map, Name, Time, GameId) values ('{}','{}','{:.2f}','{}')", map_name, name, time, uuid);

				char *errmsg;

				std::string select = std::format("select Time from record_race where Name='{}' and Map='{}' and Time={}", name, map_name, time);

				rc = sqlite3_exec(conndb, select.c_str(), callback, 0, &errmsg);

				if(execute)
					rc = sqlite3_exec(conndb, insert.c_str(), 0, 0, &errmsg);
			}
		}
	}
...
void IGameController::OnPlayerConnect(CPlayer *pPlayer)
{
	...
	// acquire client name for greeting

	std::string nach = Server()->ClientName(ClientId);
	std::string ende;
	...
	if(!Server()->ClientPrevIngame(ClientId))
	{
		...
		// ddrace player data link

		std::string conn = "https://ddnet.org/players/?json2=" + nach;

		nlohmann::json spieler;

		spieler = get_json_from_url(conn);

		std::string ende;

		// check if JSON is empty meaning players has no maps completed

		if(spieler.size() != 0)
		{
			std::cout << dbused;

			// launching async method to acquire data

			unsigned char *uuid = GameServer()->GameUuid().m_aData;
			std::async(std::launch::async, Datenbank, spieler, nach, *uuid);
		...
		{
			ende = "Über den Spieler " + nach + " ist nichts verzeichnet";

			GameServer()
				->Console()
				->Print(IConsole::OUTPUT_LEVEL_STANDARD, "Fehler", ende.c_str());
		}
	}

	ende = "Hallo " + nach + ", willkommen auf meinem Server!";

	// example greeting message for newly entering players

	GameServer()->SendChatTarget(-1, ende.c_str());
